...
//	public ResponseEntity<MyError> contraintViolated(jakarta.validation.ConstraintViolationException cve,WebRequest wr){
//		MyError err = new MyError(LocalDateTime.now(),cve.getConstraintViolations().stream().map(ConstraintViolationException::getMessage).collect(Collectors.toList()).get(0).toString(), wr.getDescription(false));
//		ResponseEntity<MyError> re = new ResponseEntity<>(err,HttpStatus.BAD_REQUEST);
//		return re;
//	}
